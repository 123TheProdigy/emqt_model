...
for (n in 1:10) {
  ...
  for (i in 1:100) {
    ...
    Mom1_pos <- sapply(Informed_pos_set10, function(x) as.numeric(x[i]))
    Mom2_pos <- sapply(NoisyInformed1_pos_set10, function(x) as.numeric(x[i]))
    Mom3_pos <- sapply(NoisyInformed2_pos_set10, function(x) as.numeric(x[i]))
    Mom4_pos <- sapply(Noisy1_pos_set10, function(x) as.numeric(x[i]))
    Mom5_pos <- sapply(Noisy2_pos_set10, function(x) as.numeric(x[i]))
    Mom6_pos <- sapply(Noisy3_pos_set10, function(x) as.numeric(x[i]))
    
    # Calculate the average of True and Expected values for the i-th position
    average_MM_pos[i, n] <- mean(MM_pos, na.rm = TRUE)
    average_Mom1_pos[i, n] <- mean(Informed_pos, na.rm = TRUE)
    average_Mom2_pos[i, n] <- mean(NoisyInformed1_pos, na.rm = TRUE)
    average_Mom3_pos[i, n] <- mean(NoisyInformed2_pos, na.rm = TRUE)
    average_Mom4_pos[i, n] <- mean(Noisy1_pos, na.rm = TRUE)
    average_Mom5_pos[i, n] <- mean(Noisy2_pos, na.rm = TRUE)
    average_Mom6_pos[i, n] <- mean(Noisy3_pos, na.rm = TRUE)
  }
}
